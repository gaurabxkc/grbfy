	...
	for {
		...
		if playlistID == "YOUR MUSIC" {
			...
			query := url.Values{
				...
				"fields": {"items(item(id,name,type,uri,artists(id,name),album(name,release_date),show(name),release_date,duration_ms,track_number,is_playable,restrictions(reason))),total"},
			}
